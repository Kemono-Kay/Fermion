...
module.exports = function (createDocument, standardizeColor) {
  ...
  /* const createDocument = function () {
    return doc.implementation.createDocument(null, 'root')
  }
  ...
  class BBCodeParser {
    ...
    setBBCode (str, settings = {}) {
      ...
      while (true) {
        ...
        if (match === null) break
        const tag = tags[match[this.MATCH.NAME]]
        const begin = str.slice(0, match.index)
        const end = str.slice(match.index + match[this.MATCH.ALL].length)

        // Unknown tag
        if (!tag) {
          this.errors.push([`Unknown tag [${match[this.MATCH.NAME]}]`, currentNode])
          currentNode.lastChild.textContent = begin
          try {
            const el = this.dom.createElement(match[this.MATCH.NAME])
            el.setAttribute('unknown-unhandled', true)
            el.setAttribute('unknown', true)
            el.setAttribute('tag-name', match[this.MATCH.NAME])
            el.setAttribute('original-text', match[0])
            if (match[this.MATCH.ARG]) el.setAttribute('tag-arg', match[this.MATCH.ARG])
            el.setAttribute('tag-close', Boolean(match[this.MATCH.CLOSE]))
            currentNode.appendChild(el)
          } catch (err) {
            this.errors.push([`Invalid tag tag [${match[this.MATCH.NAME]}]`, currentNode])
            currentNode.appendChild(this.dom.createTextNode(match[this.MATCH.NAME]))
          }
        ...
        if (match[this.MATCH.CLOSE] && currentNode.tagName.toLowerCase() !== tag.name) continue
        if (settings.parseHackTags && match[this.MATCH.CLOSE] && match[this.MATCH.ARG]) {
          currentNode = tag.properties.handleClosingArg(match[this.MATCH.ARG], currentNode)
        }

        // Close vanilla/custom tags (not hack tags)
        if (match[this.MATCH.CLOSE]) {
          currentNode.lastChild.textContent = begin
        ...
        // Find hack tag
        if (settings.parseHackTags && !tag.properties.takesArgument && match[this.MATCH.ARG]) {
          const hackElTag = tags[match[this.MATCH.ARG].split('=')[0].replace('/', '').trim()]
          if (hackElTag) {
            const hackEl = this.dom.createElement(hackElTag.name)
            if (hackElTag.properties.takesArgument && match[this.MATCH.ARG].split('=')[1]) {
              hackEl.setAttribute('arg', match[this.MATCH.ARG].split('=')[1].trim())
            }
            if (hackElTag.properties.requiresClosing) {
              hackEl.setAttribute('hacktag-unhandled', true)
              hackEl.setAttribute('tag-close', match[this.MATCH.ARG].split('=')[0].trim()[0] === '/')
            }
        ...
        if (tag.properties.takesArgument && match[this.MATCH.ARG]) normalEl.setAttribute('arg', match[this.MATCH.ARG])
  ...
  return { BBCodeParser, Tag, urlValidationRegex, urlDomainRegex, tags, RGBToHSL, addTag }
}
