...
const { BBCodeParser } = (function () {
  ...
  class Tag {
    constructor (name, settings = {}) {
      ...
      // Convert to BBCode
      this.convertToBBCode = settings.convertToBBCode !== undefined ? settings.convertToBBCode : (arg) => true
    }
  ...
  */
  ...
  const validBBCodeTags = [
    ...
    new Tag('right'),
    new Tag('center'),
    new Tag('justify', { takesArgument: true }), // Not implemented
    new Tag('code', { disallowedChildren: 'all', takesArgument: true }), // Not implemented ```x```, `x`
    /*
    possibly table, too
    */
    ...
    new Tag('collapse', { takesArgument: true, requiresContent: false }), // [x]{y}
    new Tag('quote', { takesArgument: true }), // > x
    new Tag('indent', { takesArgument: true }), // TODO: if alignment switches to right, indent on the right side. Justify/center goes both ways.
    new Tag('hr', { takesArgument: true, requiresClosing: false }) // ---, ***, - - -, * * *, etc.
  ...
  class BBCodeParser {
      ...
      while (true) {
        ...
        currentTag.disallowedChildren.includes(match[MATCH.NAME]))
          ...
          validBBCodeTags.find(tag => tag.name === match[MATCH.ARG].split('=')[0])
            ...
            const el = this.dom.createElement(match[MATCH.ARG].split('=')[0])
            const arg = match[MATCH.ARG].split('=')[1]
            if (arg && hackTag.takesArgument) el.setAttribute('arg', arg)
            currentNode.appendChild(el)
            if (hackTag.requiresClosing) currentNode = el
            currentNode.appendChild(
              this.dom.createTextNode(
              `[${match[MATCH.NAME]}]` + str.slice(match.index + match[MATCH.ALL].length)))
    ...
    getBBCode (settings = {}) {
      settings = { ...this.settings, ...settings }
      const toBBCode = function (el) {
        return [...el.childNodes].map(node => {
          if (node.nodeType === Node.TEXT_NODE) {
            return node.textContent
          } else {
            const tag = validBBCodeTags.find(tag => tag.name === node.tagName.toLowerCase())
            if (!tag && settings.swallowUnknownTags) return toBBCode(node)
            const arg = node.getAttribute('arg')
            if (settings.parseHackTags && !webclientBBCodeTags.includes(tag)) {
              return `[i=${tag.name + (arg ? `=${arg}` : '')}][/i]${tag.requiresClosing ? `${toBBCode(node)}[i=/${tag.name}][/i]` : ''}`
            }
            return `[${tag.name + (arg ? `=${arg}` : '')}]${tag.requiresClosing ? `${toBBCode(node)}[/${tag.name}]` : ''}`
            // return toBBCode(node)
          }
        }).join('')
      }
      return toBBCode(this.dom.firstElementChild)
    }
